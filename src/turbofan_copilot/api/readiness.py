...
CLIENT_API_KEY_CHECK = "client_api_key"
...
def check_client_api_key(settings: Settings) -> DependencyCheck:
    ...
    """
    if settings.client_api_key is None:
        return DependencyCheck(
            name=CLIENT_API_KEY_CHECK, ok=False, detail="TURBOFAN_CLIENT_API_KEY is not set"
        )
    return DependencyCheck(name=CLIENT_API_KEY_CHECK, ok=True, detail="configured")
