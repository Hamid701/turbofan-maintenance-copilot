...
```bash
curl -X POST localhost:8000/v1/query -H "content-type: application/json"   -H "X-API-Key: $TURBOFAN_QUERY_API_KEY" \
...
| `POST /v1/query` | `X-API-Key` | Grounded answer with citations, or a structured abstention |
| `POST /v1/query/stream` | `X-API-Key` | The same answer, preceded by SSE stage markers |
| `POST /v1/feedback` | `X-API-Key` | Record a reader's up/down verdict on an earlier answer |
...
The `/v1` endpoints need the key set in `TURBOFAN_QUERY_API_KEY`, sent as an `X-API-Key` header:
every query spends the configured OpenAI key, and feedback writes to the database. A wrong or
missing header gets 401. **With no key configured they answer 503**, so a deployment that forgets
the variable is closed rather than open. The key is compared in constant time. It is one shared
...
set `TURBOFAN_OPENAI_API_KEY`, and choose a long random `TURBOFAN_QUERY_API_KEY`. Then:
...
- `GET /ready`: **readiness**. Can this instance serve queries? It checks the
  ...
  disk, that a generation key is configured, and that the API key callers must send
  is configured, and answers 503 if any of them fails. A failure means *stop sending traffic here*, not *restart me*.
...
{"status": "ready", "checks": [
  ...
  {"name": "api_key",         "ok": true, "detail": "configured"}]}
```
