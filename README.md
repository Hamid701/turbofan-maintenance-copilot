...
curl -X POST localhost:8000/v1/query -H "content-type: application/json" \
  -H "X-API-Key: $TURBOFAN_CLIENT_API_KEY" \
...
The `/v1` endpoints need the key set in `TURBOFAN_CLIENT_API_KEY`, sent as an `X-API-Key` header:
...
set `TURBOFAN_OPENAI_API_KEY`, and choose a long random `TURBOFAN_CLIENT_API_KEY`. Then:
...
{"status": "ready", "checks": [
  ...
  {"name": "client_api_key",  "ok": true, "detail": "configured"}]}
```
